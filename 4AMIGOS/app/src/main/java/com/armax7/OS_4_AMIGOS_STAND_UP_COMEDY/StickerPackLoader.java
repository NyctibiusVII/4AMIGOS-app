...
import static com.armax7.OS_4_AMIGOS_STAND_UP_COMEDY.StickerContentProvider.ANDROID_APP_DOWNLOAD_LINK_IN_QUERY;
import static com.armax7.OS_4_AMIGOS_STAND_UP_COMEDY.StickerContentProvider.AVOID_CACHE;
import static com.armax7.OS_4_AMIGOS_STAND_UP_COMEDY.StickerContentProvider.IOS_APP_DOWNLOAD_LINK_IN_QUERY;
import static com.armax7.OS_4_AMIGOS_STAND_UP_COMEDY.StickerContentProvider.LICENSE_AGREENMENT_WEBSITE;
import static com.armax7.OS_4_AMIGOS_STAND_UP_COMEDY.StickerContentProvider.PRIVACY_POLICY_WEBSITE;
import static com.armax7.OS_4_AMIGOS_STAND_UP_COMEDY.StickerContentProvider.PUBLISHER_EMAIL;
import static com.armax7.OS_4_AMIGOS_STAND_UP_COMEDY.StickerContentProvider.PUBLISHER_WEBSITE;
import static com.armax7.OS_4_AMIGOS_STAND_UP_COMEDY.StickerContentProvider.STICKER_FILE_EMOJI_IN_QUERY;
import static com.armax7.OS_4_AMIGOS_STAND_UP_COMEDY.StickerContentProvider.STICKER_FILE_NAME_IN_QUERY;
import static com.armax7.OS_4_AMIGOS_STAND_UP_COMEDY.StickerContentProvider.STICKER_PACK_ICON_IN_QUERY;
import static com.armax7.OS_4_AMIGOS_STAND_UP_COMEDY.StickerContentProvider.STICKER_PACK_IDENTIFIER_IN_QUERY;
import static com.armax7.OS_4_AMIGOS_STAND_UP_COMEDY.StickerContentProvider.STICKER_PACK_NAME_IN_QUERY;
import static com.armax7.OS_4_AMIGOS_STAND_UP_COMEDY.StickerContentProvider.STICKER_PACK_PUBLISHER_IN_QUERY;
import static com.armax7.OS_4_AMIGOS_STAND_UP_COMEDY.StickerContentProvider.IMAGE_DATA_VERSION;
...
class StickerPackLoader {
    ...
    static ArrayList<StickerPack> fetchStickerPacks(Context context) throws IllegalStateException {
        ...
        if (cursor == null) {
            throw new IllegalStateException("não foi possível buscar no provedor de conteúdo, " + BuildConfig.CONTENT_PROVIDER_AUTHORITY);
        }
        ...
        for (StickerPack stickerPack : stickerPackList) {
            if (identifierSet.contains(stickerPack.identifier)) {
                throw new IllegalStateException("Os identificadores dos pacotes de adesivos devem ser exclusivos, há mais de um pacote com identificador:" + stickerPack.identifier);
            } else {
                identifierSet.add(stickerPack.identifier);
            }
        }
        if (stickerPackList.isEmpty()) {
            throw new IllegalStateException("Deve haver pelo menos um pacote de adesivos no aplicativo");
        }
        for (StickerPack stickerPack : stickerPackList) {
    ...
    private static List<Sticker> getStickersForPack(Context context, StickerPack stickerPack) {
        ...
        for (Sticker sticker : stickers) {
            ...
            try {
                ...
                if (bytes.length <= 0) {
                    throw new IllegalStateException("O arquivo do ativo está vazio, pacote: " + stickerPack.name + ", adesivo: " + sticker.imageFileName);
                }
                sticker.setSize(bytes.length);
            } catch (IOException | IllegalArgumentException e) {
                    throw new IllegalStateException("O arquivo de ativos não existe. pacote: " + stickerPack.name + ", adesivo: " + sticker.imageFileName, e);
            }
        }
        return stickers;
    }
    ...
    static byte[] fetchStickerAsset(@NonNull final String identifier, @NonNull final String name, ContentResolver contentResolver) throws IOException {
        try (final InputStream inputStream = contentResolver.openInputStream(getStickerAssetUri(identifier, name));
            ...
            if (inputStream == null) {
                throw new IOException("não consegue ler o material da etiqueta:" + identifier + "/" + name);
            }
            int read;
